...
public class ZooKeeperJob {
	...
	public static void doAction(int client) throws Exception {
		String host1 = "192.168.2.5:2181";
		String host2 = "192.168.2.6:2181";
		String host3 = "192.168.2.7:2181";
	...
	public static void initQueue(ZooKeeper zk) throws KeeperException,
		...
		if (zk.exists(QUEUE, false) == null) {
			...
			zk.create(QUEUE, QUEUE.getBytes(), Ids.OPEN_ACL_UNSAFE,
	...
	public static void doPurchase(ZooKeeper zk) throws Exception {
		if (zk.exists(PURCHASE, false) == null) {
			...
			zk.create(PURCHASE, PURCHASE.getBytes(), Ids.OPEN_ACL_UNSAFE,
					CreateMode.PERSISTENT);
		} else {
	...
	public static void doSell(ZooKeeper zk) throws Exception {
		if (zk.exists(SELL, false) == null) {
			...
			zk.create(SELL, SELL.getBytes(), Ids.OPEN_ACL_UNSAFE,
					CreateMode.PERSISTENT);
		} else {
	...
	public static void doOther(ZooKeeper zk) throws Exception {
		if (zk.exists(OTHER, false) == null) {
			...
			zk.create(OTHER, OTHER.getBytes(), Ids.OPEN_ACL_UNSAFE,
					CreateMode.PERSISTENT);
		} else {
	...
	public static void isCompleted(ZooKeeper zk) throws Exception {
		...
		if (length >= size) {
			...
			Profit.profit();
			zk.create(PROFIT, PROFIT.getBytes(), Ids.OPEN_ACL_UNSAFE,
